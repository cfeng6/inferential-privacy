...
class gigan_generator(nn.Module):
    ...
    def forward(
        ...
        _, styles = self.style_network(x = noise, text_latent= global_text_tokens)
        w_z = noise + self.lr_alpha * (noise - styles) / torch.norm(noise - styles, 2)
        _, styles = self.style_network(x = w_z, text_latent= global_text_tokens)
...
class GigaGAN(nn.Module):
    def __init__(self, in_channels, dec_channels, latent_size, private_classes, lr_alpha):
        super(GigaGAN, self).__init__()
        self.decoder = gigan_generator(lr_alpha=lr_alpha)
...
# out = g(noise = x, global_text_tokens = x1)  
# print(out.size())
# enc = GigaGAN(3, 32, 512, 307)
# print(enc.decoder)
# x = torch.randn(16,3,128,128)
# y = torch.ones((16, 1)).long()
# z, out = enc(x, y, is_private=False)
# print(z.size())
# print(out.size())
# print(torch.max(out))
# print(torch.min(out))
...
# resolutions = resolutions.long().tolist()
# print(resolutions)
# dim_layers = (2 ** (torch.arange(num_layers) + 1)) * 2
# dim_layers = torch.flip(dim_layers, (0,))
# dim_layers = F.pad(dim_layers, (1, 0), value = dim_latent)
# print(dim_layers)
# dim_layers = dim_layers.tolist()
